...
“이벤트가 **어떻게 흘러가고(전파)**, **브라우저가 기본 동작을 할지 말지**를 개발자가 직접 제어하는 도구들”입니다.
...
### 3-3. `event.cancelable`과 `event.defaultPrevented` O
...
(브라우저가 리스너 실행을 기다리지 않고 스크롤을 먼저 진행해도 안전하다고 판단)

 **꼭 기억할 기본값**: 아래 조합은 **옵션을 주지 않아도 `passive`가 기본 `true`** 입니다.

* 대상: `window`, `document`, `document.body`
* 이벤트: `touchstart`, `touchmove`, `wheel`, `mousewheel`

```js
// ❌ 막히지 않음 — 이 조합은 기본이 passive: true
...
그 외 일반 요소(`div.addEventListener('wheel', ...)`)나 다른 이벤트 타입은 `passive`의 기본값이 `false`입니다.
...
* **`capture` 값도 등록할 때와 같아야 합니다.** 리스너의 신원은 `(type, listener, capture)` 세 가지로 결정되기 때문입니다.
  (`once`, `passive`, `signal`은 매칭에 영향을 주지 않습니다.)

```js
el.addEventListener('click', handler, { capture: true });

el.removeEventListener('click', handler);                    // ❌ 안 지워짐
...
* 모달 내부를 클릭하면 **닫히지 않게** 하는 패턴입니다.
 ...
 다만 이 구현은 **모달 내부의 모든 클릭 전파를 죽여버립니다.** 상위에 걸어둔 위임 리스너
(전역 클릭 로깅, 드롭다운 닫기, **React 루트 컨테이너의 이벤트 위임** 등)까지 함께 못 받게 되므로,
전파를 끊는 대신 **타깃을 검사하는 방식**이 더 안전합니다.

```js
// ✅ 전파를 막지 않는 버전
backdrop.addEventListener('click', (event) => {
  if (event.target !== event.currentTarget) return; // 내부에서 올라온 클릭이면 무시
  backdrop.style.display = 'none';
});
...
> 참고: 요즘은 `<dialog>` 엘리먼트의 `showModal()` + `::backdrop`을 쓰면
> 포커스 트랩과 `Esc` 닫기까지 브라우저가 처리해 줍니다.
...
→ 이 경우 **기본 동작은 막히지 않고** 콘솔 경고만 뜹니다.

그리고 더 헷갈리는 지점 — `window`/`document`/`body` + `wheel`·`touchstart`·`touchmove`에서는
**옵션을 생략해도 `passive: true`가 기본**이라, 아래 코드도 스크롤을 막지 못합니다.

```js
// ❌ 옵션을 안 줘도 window의 wheel은 passive: true가 기본
