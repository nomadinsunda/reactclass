...
컴파일 후 — ② React 17+ 부터 기본값인 **Automatic 런타임(새 JSX Transform)**:
...
> Vite + React 19 환경은 Automatic 런타임이 기본이라, 컴파일러가 `react/jsx-runtime` 에서 `jsx`/`jsxs` 를 **자동으로 import** 합니다.
...
## ③ React 19: `ref` 도 그냥 props

React 19부터 함수 컴포넌트는 **`ref` 를 일반 prop처럼 직접 받을 수 있습니다.** `forwardRef` 로 감쌀 필요가 없어졌습니다.
...
`forwardRef` 도 여전히 동작하지만, 신규 코드에서는 쓰지 않아도 됩니다.
