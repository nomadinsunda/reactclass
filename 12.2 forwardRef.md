...
> # React 19 정정 — 먼저 읽어주세요
>
> **React 19부터 함수 컴포넌트는 `ref`를 “일반 prop”으로 직접 받습니다.**
> 따라서 **`forwardRef`는 더 이상 필요하지 않으며, 향후 deprecated 될 예정**입니다.
>
> ```jsx
> // O React 19 — 이걸로 충분합니다
> function Input({ ref, ...props }) {
>   return <input ref={ref} {...props} />;
> }
>
> // React 18 이하 — forwardRef가 필수였습니다
...
> | -------------- | ---------------------------- | ------------------------------- |
> | ref 받는 위치      | 렌더 함수의 **두 번째 인자**           | **`props.ref`** (구조 분해로 꺼내 씀)   |
> | `forwardRef`   | **필수**                       | 불필요 (동작은 하지만 deprecated 예정)     |
> | HOC에서 ref 전달   | HOC 내부에서 다시 `forwardRef` 필요  | `props`에 `ref`가 실려 있어 그대로 통과됨   |
> | TypeScript     | `forwardRef<TInstance, TProps>` | props 타입에 `ref?: Ref<T>` 를 포함  |
>
> **그럼 아래 내용은 왜 남겨두나요?**
> 실무 코드베이스, 대부분의 UI 라이브러리, 그리고 인터넷의 거의 모든 예제가 아직 `forwardRef`로 작성되어 있기 때문입니다.
> 아래 1~10장은 **레거시 코드를 읽고 유지보수하기 위한 설명**으로 읽어주시고,
> **새로 작성하는 코드에는 React 19 방식(§11)을 사용**하세요.

---

## 1. 왜 `forwardRef`가 필요했는가? (문제 상황부터)

> 이 장부터 10장까지는 **React 18 이하 기준**입니다. React 19 방식은 **§11**을 보세요.
...
* 이제 부모는 `inputRef.current`로 **진짜 `<input>` DOM**에 접근 가능 O

---

## 3. `props`와 `ref`의 차이: 왜 ref는 두 번째 인자로 오나? *(React 18 이하)*
...
## 6. HOC(고차 컴포넌트)와 `forwardRef`: ref가 사라지는 문제 해결
...
### 9-1. `ref`를 props로 받아버리는 실수 *(React 18 이하 한정)*

```jsx
function MyComponent(props) {
  // ❌ React 18 이하: 이렇게 하면, 부모가 준 ref는 여기 안 옴
  const { ref, ...rest } = props;
  ...
}
```

* React 18 이하에서 `ref`는 JSX에서 **props로 전달되지 않습니다** (별도 필드)
* `forwardRef`를 쓰지 않으면 함수 컴포넌트는 ref를 받을 수 없습니다.

> ### O React 19에서는 이게 **정답**입니다
>
> ```jsx
> function MyComponent({ ref, ...rest }) {
> return <input ref={ref} {...rest} />; // O 정상 동작
> }
> ```
>
> 즉 **“실수”였던 코드가 React 19에서는 권장 패턴**이 되었습니다.
> 버전에 따라 정반대로 동작하므로, 예제를 볼 때 **어느 버전 기준인지 반드시 확인**하세요.
...
* **React 18 이하** X : 경고 `Function components cannot be given refs...` 가 뜨고 `ref.current`는 `null`
  → 반드시 `forwardRef`로 감싸야 함
* **React 19** : 경고는 사라지지만, `Child`가 **받은 `ref`를 어딘가에 붙이지 않으면** 여전히 `ref.current`는 `null`입니다.

  ```jsx
  // ✅ React 19: 받은 ref를 실제로 DOM에 연결해야 함
  function Child({ ref }) {
    return <div ref={ref}>Child</div>;
  }
  ```

* React 19의 함정: **“에러가 안 나니까 동작하는 줄 안다”**. ref를 받아서 **연결하는 책임은 컴포넌트 작성자에게** 있습니다.

### 9-3. `forwardRef`와 React 19 방식을 **섞어 쓰기**

```jsx
// ❌ 이중 처리 — 혼란만 가중
const Bad = forwardRef(function Bad({ ref: refFromProps }, ref) {
  ...
});
```

* `forwardRef`로 감싼 컴포넌트에서는 ref가 **두 번째 인자**로 옵니다. `props.ref`는 `undefined`입니다.
* 한 프로젝트 안에서는 **한 가지 방식으로 통일**하세요. (React 19라면 prop 방식)

### 9-4. `useImperativeHandle` 남용
...
> 아래는 **“ref를 자식에게 넘겨야 하는 상황”** 목록입니다.
> **React 18 이하**라면 이때 `forwardRef`가 필요하고,
> **React 19**라면 같은 상황에서 `function C({ ref })` 로 해결하면 됩니다.

O ref 전달이 필요한 대표 상황들:
...
## 11. O React 19 방식: `forwardRef` 없이 쓰기

### 11-1. 기본형 — `ref`는 그냥 prop입니다

```jsx
// React 19
function TextInput({ label, ref, ...rest }) {
  return (
    <label style={{ display: 'block', marginBottom: '8px' }}>
      <span style={{ display: 'block', marginBottom: '4px' }}>{label}</span>
      <input ref={ref} {...rest} />
...
> **주의**: `{...rest}`로 스프레드할 때 `ref`가 `rest`에 남아 있으면 안 됩니다.
> 위처럼 **구조 분해에서 `ref`를 먼저 꺼내면** `rest`에는 포함되지 않으므로 안전합니다.

### 11-2. `useImperativeHandle`과 함께 (React 19)
...
* `forwardRef`가 사라졌을 뿐, **`useImperativeHandle`의 사용법은 그대로**입니다.
* 첫 번째 인자에 넘기는 `ref`가 **두 번째 인자에서 prop으로** 바뀐 것뿐입니다.

### 11-3. HOC도 훨씬 간단해집니다

```jsx
// React 19: HOC 안에서 forwardRef를 다시 쓸 필요가 없음
function withLogger(WrappedComponent) {
  return function WithLogger(props) {
    console.log('렌더링:', WrappedComponent.displayName || WrappedComponent.name);
    return <WrappedComponent {...props} />; // ref가 props에 포함되어 그대로 통과
  };
}
```

* `ref`가 `props`의 일부이므로 `{...props}` 스프레드만으로 **자동 전달**됩니다.
* §6에서 설명한 “HOC에서 ref가 끊기는 문제”가 **구조적으로 사라집니다.**

### 11-4. TypeScript (React 19)
...
type TextInputProps = React.InputHTMLAttributes<HTMLInputElement> & {
  label: string;
  ref?: Ref<HTMLInputElement>;   // 👈 props 타입에 ref를 그냥 선언
};
...
* `forwardRef<TInstance, TProps>` 제네릭이 필요 없어집니다.
* `displayName`을 따로 지정할 필요도 없습니다. (일반 함수라 이름이 그대로 잡힘)

### 11-5. 마이그레이션 체크리스트

1. `forwardRef(` 를 전역 검색 → 렌더 함수의 두 번째 인자 `ref`를 **첫 번째 인자의 구조 분해**로 이동
2. `React.forwardRef` import 제거, `displayName` 수동 지정 제거
3. TS: `forwardRef<T, P>` → props 타입에 `ref?: Ref<T>` 추가
4. **콜백 ref의 암묵적 반환 점검** — `ref={el => (x.current = el)}` → `ref={el => { x.current = el; }}`
5. `element.ref` 직접 접근 코드 → `element.props.ref` 로 변경
6. codemod 활용: `npx codemod@latest react/19/migration-recipe`

> **정리**
> React 19에서 `forwardRef`는 **“없어도 되는 API”** 가 되었습니다.
> 하지만 여전히 동작하므로 **급하게 전부 고칠 필요는 없고**, 새 코드부터 prop 방식으로 작성하면 됩니다.
> 다만 라이브러리를 만든다면, 소비자의 React 버전에 따라 동작이 갈리므로 **peerDependencies와 문서에 지원 버전을 명시**하세요.
