...
```js
// 현재 렌더링 중인 컴포넌트의 Hook 목록
let currentFiber = null;

// 현재 몇 번째 Hook을 처리하고 있는가?
let hookIndex = 0;
...
function useState(initialState) {
  ...
  // 현재 위치의 Hook을 가져온다.
  const hook = getCurrentState(initialState);

  function setState(nextState) {
    hook.memoizedState = nextState;

    // 상태가 변경되었으므로 다시 렌더링
    scheduleRender(currentFiber);
  }
...
function getCurrentState(initialState) {
  ...
  // 현재 Fiber가 가지고 있는 Hook 목록에서
  // 현재 순번의 Hook을 가져온다.
  let hook = currentFiber.memoizedState[hookIndex];


  // 최초 렌더링이라 해당 Hook이 아직 없다면
  // 새로운 Hook을 만든다.
  if (hook === undefined) {
