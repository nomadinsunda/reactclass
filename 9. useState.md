...
1. `useState(초기값)`은 **배열을 반환**합니다.
2. **첫 번째 요소**: 현재 상태 값 (`count`)
3. **두 번째 요소**: 상태를 변경하는 함수 (`setCount`)
4. `setCount`를 호출하면 → React가 **이 컴포넌트를 다시 렌더링**합니다.
5. 다시 렌더링할 때 `useState`는 **이전에 저장해둔 상태 값**을 꺼내서 `count`에 넣어줍니다.

TypeScript로 보면 시그니처는 대략 이렇게 생겼다고 볼 수 있습니다:
...
`useState`의 초기값은 값도 될 수 있지만, **“함수”로 넘길 수도 있습니다.**
...
* 컴포넌트가 **렌더될 때마다** `expensiveComputation()`이 호출됩니다.
* 하지만 실제로 `useState`의 초기값은 **첫 렌더에서만 사용**되는데도, 매번 계산이 일어납니다.

이럴 때는 다음과 같이 **함수 형태**로 넘기면 됩니다.
...
* `useState`는 초기 렌더 시에만 이 함수를 호출해서 초기 상태를 구합니다.
* 그 이후 렌더에서는 이 함수가 호출되지 않습니다.

> ✅ 정리: 초기화가 무겁고 비용이 큰 계산일 때는
> `useState(() => 무거운계산())` 형태로 쓰시면 성능상 유리합니다.
