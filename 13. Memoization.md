...
4. `useMemo` 깊게 보기
5. `useCallback` 깊게 보기
6. 내부적으로 어떻게 동작하는지(hook.memoizedState와 연결)
7. 언제 쓰고, 언제 쓰지 말아야 하는지 체크리스트 O
...
그래서 리액트는 **“굳이 다시 렌더 안 해도 되면 좀 건너뛰자”** 라는 최적화 장치를 줍니다.
그게 바로:
...
### 시작하기 전에 – 메모이제이션은 공짜가 아닙니다

가장 먼저 바로잡아야 할 오해가 이것입니다.

> **“일단 다 감싸두면 손해는 없겠지”는 사실이 아닙니다.**

세 도구 모두 다음 비용을 **매 렌더마다** 지불합니다.

* **비교 비용** – `React.memo`는 props 개수만큼, `useMemo`/`useCallback`은 deps 개수만큼 `Object.is` 비교를 돕니다.
* **메모리 비용** – 이전 값·이전 함수·이전 deps를 계속 붙잡고 있으므로 GC 대상이 되지 않습니다.
* **복잡도 비용** – deps 관리 실수는 곧 stale 값 버그입니다.

즉 메모이제이션은 **“연산을 메모리 + 비교 비용과 맞바꾸는 거래”**입니다.
이 거래가 이득인지는 **측정하기 전에는 알 수 없습니다.**

### 메모이제이션은 “보장”이 아니라 “힌트”입니다

또 하나 자주 오해하는 지점입니다.

* 리액트 공식 문서는 `useMemo`/`useCallback`의 캐시를 **버릴 수 있다(may throw away the cache)**고 명시합니다.

  * 예: 화면 밖(offscreen)으로 밀려난 서브트리의 메모리를 회수하는 경우
* `React.memo`도 **“항상 렌더를 건너뛴다”는 보장이 아닙니다.**

  * 리액트가 필요하다고 판단하면 그냥 다시 렌더할 수 있습니다.

 그래서 메모이제이션은 **성능 최적화 수단**이지,
**“이 코드는 절대 다시 실행되지 않는다”는 정합성 보장 수단이 아닙니다.**
계산이 반드시 한 번만 실행되어야 한다면(예: WebSocket/인스턴스 생성),
그건 `useMemo`가 아니라 `useState(() => ...)`의 지연 초기화나 `useRef`로 풀어야 합니다.
...
#### 반드시 함께 기억할 두 방향의 연결 고리

`React.memo`와 `useMemo`/`useCallback`은 **둘 다 갖춰져야 의미가 생깁니다.**

**(1) 자식이 `React.memo`가 아니면, `useCallback`은 아무 효과가 없습니다.**

```jsx
// Child는 그냥 함수 컴포넌트 (memo 아님)
function Child({ onClick }) { /* ... */ }

function Parent() {
  const onClick = useCallback(() => {}, []); // 😴 아무 의미 없음
  return <Child onClick={onClick} />;        // 부모가 렌더되면 Child도 무조건 렌더
}
```

부모가 렌더되면 자식도 렌더되는 것이 리액트의 기본 동작이고,
그 사슬을 끊는 것은 **`React.memo`(또는 `children`으로 주입하기)** 뿐입니다.
자식이 memo가 아닌 상태에서의 `useCallback`은 **비용만 내고 이득은 0**입니다.

**(2) 자식이 `React.memo`여도, props 중 하나라도 리터럴이면 전부 무력화됩니다.**

```jsx
<MemoChild
  onClick={handleClick}         // ✅ useCallback으로 고정
  style={{ margin: 8 }}         // ❌ 렌더마다 새 객체 → 여기서 이미 비교 실패
  items={items.filter(Boolean)} // ❌ 렌더마다 새 배열
  render={() => <Icon />}       // ❌ 렌더마다 새 함수
/>
```

`React.memo`의 props 비교는 **얕은 비교(shallow)**이므로,
props 목록에서 **단 하나라도** 참조가 달라지면 나머지를 아무리 고정해도 렌더를 건너뛰지 못합니다.
이때 남는 것은 **비교 비용뿐**이라 오히려 손해입니다.

 그래서 `React.memo`를 붙였다면 그 컴포넌트로 들어가는 **props 전부**의 참조 안정성을 점검해야 합니다.
점검할 자신이 없다면 **애초에 붙이지 않는 편**이 낫습니다.
...
## 6. 내부 동작 – hook.memoizedState와 메모이제이션

앞서 「9.1 Hook Memory Slot」에서 다룬 `hook.memoizedState`와도 직접 연결되는 부분입니다.
...
### 8.3 진짜 최적화 순서 – 메모이제이션은 “마지막 카드”

이 순서를 지키면 실무에서는 **대부분 메모이제이션까지 가지 않고 끝납니다.**

**1단계. 측정한다**

* React DevTools의 **Profiler** 탭에서 녹화 → 실제로 느린 커밋을 찾습니다.
* Profiler 설정에서 **“Record why each component rendered”**를 켜면
  각 컴포넌트가 *왜* 렌더됐는지(props 변경 / state 변경 / 부모 렌더 / 컨텍스트 변경)를 알려줍니다.
* 체감이 아니라 **ms 숫자**로 병목을 특정합니다.

**2단계. 렌더 원인을 파악한다**

* 렌더가 **잦은 것**이 문제인지, 렌더 **한 번이 무거운 것**이 문제인지 구분합니다.
* 전자는 구조 문제, 후자는 계산량/DOM 양의 문제입니다. 처방이 완전히 다릅니다.

**3단계. 구조를 고친다 (여기서 대부분 해결됩니다)**

* **state를 아래로 내리기**: 자주 바뀌는 state를 그것을 실제로 쓰는 작은 컴포넌트로 이동
* **`children`으로 넘기기**: 부모가 렌더돼도 `children`으로 받은 엘리먼트는 **이전과 같은 엘리먼트 객체**이므로 리액트가 해당 서브트리의 리렌더를 건너뜁니다

  ```jsx
  // ❌ count가 바뀔 때마다 HeavyTree까지 렌더
  function Page() {
  ...
  // ✅ state를 Wrapper로 내리고, HeavyTree는 children으로 주입
  function Page() {
    return (
      <Wrapper>
        <HeavyTree />  {/* Page는 리렌더되지 않으므로 이 엘리먼트도 그대로 */}
      </Wrapper>
    );
  }
  ...
  이 방법은 `React.memo` 없이 리렌더를 끊습니다. **비교 비용이 0**이라 메모이제이션보다 우월합니다.
* 그 밖에: 컨텍스트 분리(자주 바뀌는 값 / 안 바뀌는 값), 리스트 가상화, `key` 설계 점검

**4단계. 그래도 남으면 메모이제이션**

* 그때 비로소 `React.memo` / `useMemo` / `useCallback`을 **핫스팟에만** 투입합니다.
* 적용 후 **다시 측정**해서 실제로 좋아졌는지 확인합니다. 좋아지지 않았다면 **되돌립니다.**
...
## 11. 보너스 – React 19 시대의 메모이제이션과 React Compiler

강의에서 학생들이 가장 헷갈려 하는 부분이라 사실관계를 정확히 정리하겠습니다.

### 11.1 “React 19가 되면 useMemo/useCallback을 안 써도 된다”? – 반은 맞고 반은 틀림

* 자동 메모이제이션을 해 주는 것은 **React 19 자체가 아니라 `React Compiler`(구 React Forget)** 입니다.
* React Compiler는 **React 19에 자동으로 내장된 기능이 아닙니다.**

  * **빌드 도구에 직접 추가하는 별도의 opt-in 도구**입니다. (Babel 플러그인 / Vite·Next.js 설정)
  * 설치하지 않으면 **아무 일도 일어나지 않습니다.** 지금까지 배운 수동 메모이제이션이 그대로 필요합니다.
* 즉 “React 19 = 자동 최적화”가 아니라
  **“React 19 + React Compiler를 켰을 때 = 자동 최적화”** 입니다.
...
* 컴포넌트/훅 코드를 분석해서 **어떤 값이 어떤 입력에 의존하는지** 계산하고,
* 렌더 결과·계산 값·함수·JSX 엘리먼트를 **자동으로 캐시**하는 코드를 생성합니다.
* 결과적으로 우리가 손으로 붙이던 `useMemo` / `useCallback` / `React.memo` 상당 부분을 **대체**합니다.

### 11.3 전제 조건과 한계

* 컴파일러는 **“Rules of React”를 지키는 코드**에만 안전하게 동작합니다.

  * 렌더 중 부수 효과 금지, props/state 변경(mutation) 금지, 훅 규칙 준수
  * 규칙 위반이 의심되면 컴파일러는 **그 컴포넌트의 최적화를 건너뜁니다.** (조용히 스킵)
  * 그래서 `eslint-plugin-react-hooks`의 컴파일러 규칙을 함께 켜는 것이 사실상 필수입니다.
* 컴파일러를 켜도 **기존 `useMemo`/`useCallback` 코드는 그대로 동작**합니다. 급히 지울 필요는 없습니다.
* 반대로 컴파일러가 있다고 해서 **3단계(구조 개선)가 면제되지는 않습니다.**
  잘못된 state 위치, 과도한 컨텍스트 재생성, 리스트 가상화 부재는 컴파일러가 고쳐 주지 못합니다.

### 11.4 그래서 지금 우리가 할 일

1. **메모이제이션의 원리(참조 동일성, 얕은 비교, 렌더 사슬)는 계속 알아야 합니다.**
   컴파일러가 왜 최적화를 못 했는지 읽어내려면 결국 이 지식이 필요합니다.
2. 새 프로젝트라면 컴파일러 도입을 **검토**하고, 도입했다면 수동 메모이제이션은 **꼭 필요한 곳만** 남깁니다.
3. 도입하지 않았다면, 이 장의 원칙 그대로:
   **측정 → 원인 파악 → 구조 개선 → 그래도 남으면 메모이제이션.**
