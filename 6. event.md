...
> 캡처링의 시작점과 버블링의 종착점은 모두 `window`입니다.
> `event.composedPath()`가 돌려주는 배열도 `[target, …조상들, <body>, <html>, document, window]` 순서입니다.

**타깃 단계의 미묘한 규칙**
타깃 요소 *자신*에 등록된 리스너는 `capture: true`든 아니든 **전부 타깃 단계에서 실행**되고, 그 순서는 캡처/버블 여부가 아니라 **등록한 순서(registration order)**로 결정됩니다.
즉 타깃 요소 위에서는 “캡처가 항상 먼저”가 성립하지 않습니다. (이때 `event.eventPhase === 2`(AT_TARGET), 캡처 중이면 `1`, 버블 중이면 `3`)
...
* `event.composedPath()`
  ...
  * 이벤트가 통과한 노드들의 경로 배열 (타깃 → 조상들 → `document` → `window`)
  * Shadow DOM 내부에서 발생해 `composed: true`로 바깥까지 나온 이벤트는 `event.target`이 셰도우 호스트로 **리타깃팅(retargeting)**되므로, 실제 내부 타깃을 알려면 `composedPath()[0]`을 봐야 합니다.

* `event.eventPhase`

  * `1` = 캡처링, `2` = 타깃, `3` = 버블링 (흐름 디버깅에 매우 유용)

* `event.isTrusted`

  * 사용자의 실제 조작이면 `true`, `dispatchEvent()`로 만들어 쏜 이벤트면 `false`
...
## 4.3 `addEventListener` 방식 (표준, 권장) O
...
btn.addEventListener(
  ...
  handler1,
  {
    ...
    signal: controller.signal // abort() 한 번으로 일괄 해제
  }
);
...
* `boolean`을 넘기면 그건 `capture` 값입니다. → `addEventListener('click', h, true)` === `{ capture: true }`
* `capture`는 **리스너의 신원(identity)** 에 포함됩니다. 캡처로 등록했으면 제거할 때도 `removeEventListener('click', h, true)`처럼 캡처 플래그를 맞춰야 합니다. (`once`, `passive`, `signal`은 제거 시 매칭에 관여하지 않습니다.)
* `signal`(AbortController)은 컴포넌트 언마운트/라우트 전환 시 리스너를 일괄 해제하는 **현대적인 표준 방식**입니다.
...
* `passive: true` 리스너에서 `preventDefault()`를 호출하면 **아무 일도 일어나지 않고**(무시됨) 콘솔 경고만 뜹니다.
* **가장 많이 걸리는 함정**: `touchstart`, `touchmove`, `wheel`, `mousewheel`을 `window` / `document` / `document.body`에 등록하면 **옵션을 주지 않아도 `passive`가 기본 `true`** 입니다(스크롤 성능을 위한 브라우저 기본값). 이 경우 스크롤을 막으려면 **반드시 `{ passive: false }`를 명시**해야 합니다.

```js
// ❌ 스크롤이 막히지 않음 (window에서는 passive가 기본 true)
...
* `scroll` : **요소에서 발생하면 버블링되지 않습니다.** (`document`에서 발생할 때만 버블링 → `window`에서도 잡힘) / `cancelable: false`라 `preventDefault()`로 스크롤을 막을 수 없습니다. 막으려면 `wheel`/`touchmove`를 `{ passive: false }`로 잡거나 CSS `overflow`, `touch-action`을 쓰세요.
* `scrollend` : 스크롤이 멈춘 뒤 한 번 발생 (디바운스 대체용, 최신 브라우저)
* `resize` : `window`에서만 의미 있음 (요소 크기 감지는 `ResizeObserver`)
* `hashchange`
* `online` / `offline`
...
# 11. DOM 이벤트 다룰 때 자주 하는 실수 & 베스트 프랙티스 O
