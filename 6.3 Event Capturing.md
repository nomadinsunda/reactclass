...
3. **Bubbling Phase (버블링 단계)**
   ...
   * *타깃 요소* → 부모 → 조상 → `<body>` → `<html>` → `document` → `window`
   * “아래에서 위로 올라가는” 단계
...
### 6.1. 네이티브 리스너와 섞였을 때의 실제 실행 순서

React 핸들러는 **개별 DOM 노드가 아니라 루트 컨테이너에서** 실행된다는 점 때문에,
`useRef` + `addEventListener`로 직접 건 리스너와 섞이면 순서가 직관과 어긋납니다.

```
1) window / document 의 캡처 리스너
2) 루트 컨테이너의 캡처 리스너  →  여기서 React의 onClickCapture들이 위 → 아래로 실행
3) 루트 안쪽 DOM 노드들의 네이티브 리스너 (캡처 → 타깃 → 버블)
4) 루트 컨테이너의 버블 리스너  →  여기서 React의 onClick들이 아래 → 위로 실행
5) document / window 의 버블 리스너
```

정리하면:

* **`onClickCapture`는 안쪽 네이티브 리스너보다 먼저**, **`onClick`은 안쪽 네이티브 리스너보다 나중에** 실행됩니다.
* 자식 노드의 네이티브 리스너에서 `e.stopPropagation()`을 부르면 이벤트가 루트에 도달하지 못해
  **상위 컴포넌트의 `onClick`이 통째로 실행되지 않습니다.**
* 반대로 React 핸들러의 `e.stopPropagation()`은 **React 내부의 합성 전파만** 멈춥니다.
  `document`/`window`에 직접 건 리스너까지 막으려면 `e.nativeEvent.stopImmediatePropagation()`을 써야 합니다.

---

## 7. React 17+에서의 이벤트 위임 위치 변화
...
컴포넌트 트리 안에서만 놀 때는 “동작이 똑같이 보인다”고 생각하셔도 됩니다.

### 7.1. 그래도 실제로 달라진 것들 (React 17 변경의 부작용)

1. **바깥에서 건 `stopPropagation`의 효과가 달라졌습니다.**
   React 16에서는 `document`에 위임했기 때문에, 루트와 `document` 사이의 어떤 노드가
   네이티브 리스너에서 `e.stopPropagation()`을 호출하면 React 핸들러가 **아예 실행되지 않았습니다.**
   React 17+에서는 루트 컨테이너까지만 올라가면 되므로 이런 사고가 크게 줄었습니다.
   반대로 이제는 `document`에 건 네이티브 리스너가 **React 핸들러보다 항상 늦게(버블) 혹은 항상 먼저(캡처)** 실행됩니다.

2. **`onScroll`이 더 이상 버블링하지 않습니다.** (React 17부터)
   위임 대상에서 빠져 해당 요소에 직접 붙기 때문에, 상위 컴포넌트에서 자식의 스크롤을 위임 방식으로 받을 수 없습니다.

3. **`onFocus` / `onBlur`가 네이티브 `focusin` / `focusout` 기반으로 바뀌었습니다.**
   덕분에 React에서는 포커스 이벤트가 **버블링**하며, `e.nativeEvent.type`을 찍어보면 `focusin`이 나옵니다.

4. **여러 개의 React 앱/버전을 한 페이지에 얹기가 쉬워졌습니다.**
   각 루트가 자기 컨테이너 안의 이벤트만 처리하기 때문입니다.
...
## 9. SyntheticEvent와 캡처의 관계 (그리고 `event.persist`는 이제 필요 없습니다)

리액트의 `SyntheticEvent`는 브라우저 원본 이벤트를 감싼 **크로스브라우저 래퍼**이며,
원본은 `e.nativeEvent`로 언제든 꺼낼 수 있습니다.

> **오래된 자료 주의**: React 16까지는 성능을 위해 SyntheticEvent 객체를 **풀링(pooling)** 해서 재사용했고,
> 핸들러가 끝나면 프로퍼티가 `null`로 초기화되었기 때문에 비동기에서 쓰려면 `e.persist()`가 필요했습니다.
> **React 17에서 이벤트 풀링은 완전히 제거되었습니다**(18·19 동일).
> `e.persist()`는 하위 호환용으로 남아 있을 뿐 **아무 일도 하지 않는 no-op**입니다.

```jsx
function handleClick(e) {
  // React 17+ : persist() 없이도 그대로 살아 있습니다
  setTimeout(() => {
...
* 예외는 `e.currentTarget` 하나입니다. 전파가 끝나면 `null`로 되돌아가므로,
  비동기에서 필요하면 핸들러 안에서 미리 지역 변수에 담아두세요.
...
4. React 17+에서 이벤트 위임 위치가 `document` → **루트 컨테이너**로 바뀌었습니다.
   **React 트리 내부에서의** `onClickCapture` / `onClick` 의미와 순서는 그대로지만,
   `document`나 루트 바깥 요소에 네이티브 리스너를 걸어둔 코드는 **실행 순서와 `stopPropagation`의 효과가 달라질 수 있습니다.**
   (자세한 내용은 위 7.1 참고 — “React는 모든 이벤트를 `document`에 붙인다”는 설명은 React 16까지의 이야기입니다.)
