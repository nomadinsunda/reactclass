...
→ **렌더링 단계(Render/Commit) + Hook 실행 타이밍 + cleanup**
이 조합으로 라이프사이클을 생각해야 합니다.

이 글에서는:

* 렌더링 단계(Render / Commit)
* `useEffect` / `useLayoutEffect`의 실행 타이밍 ⏱
...
5. Commit 이후 effect 순서:
   ...
   * 우선 **deps가 바뀐 effect들의 cleanup**이 실행됩니다.
     이때 순서는 컴포넌트 안에 **훅을 선언한 순서(위 → 아래)** 입니다.

     * `🧹 [업데이트 cleanup] parentCount 변경 직전 ...` ← 2번 effect(`[parentCount]`)
     * `🧹 [매 렌더링 cleanup] ...` ← 3번 effect(deps 없음)
     * 1번 effect(`[]`)는 deps가 그대로이므로 cleanup되지 않습니다.
...
3. Commit Phase:
   ...
   * 이전 effect cleanup → 새 effect 콜백 실행 순서 반복:
     ...
     * `useEffect(..., [])`
       ...
       * 빈 deps 배열 → 비교할 값이 없어 항상 “변화 없음” → **다시 실행되지 않음** (mount 때 한 번만)
...
3. 이 컴포넌트에 등록된 모든 effect의 **cleanup**이 실행됩니다.
   여기서도 순서는 **훅을 선언한 순서(위 → 아래)** 입니다:

   * `🧹 [언마운트] LifecycleDemo가 화면에서 사라짐` ← 1번 effect(`[]`)
   * `🧹 [업데이트 cleanup] ...` ← 2번 effect(`[parentCount]`)
   * `🧹 [매 렌더링 cleanup] ...` ← 3번 effect(deps 없음)

> 헷갈리기 쉬운 지점
> cleanup은 “나중에 등록된 것부터” 역순으로 도는 것이 **아닙니다.**
> 마운트/업데이트/언마운트 모두 **선언 순서대로** cleanup → effect 가 실행됩니다.
> (다만 트리 전체로 보면 **자식의 cleanup이 부모보다 먼저** 실행됩니다.)
...
개발 모드에서 `<StrictMode>` 안에 렌더링하면:

* 일부 동작이 **의도적으로 두 번 실행**됩니다.

  * 컴포넌트 함수 호출 (두 번 호출해도 화면에 반영되는 결과는 한 번)
  * `useEffect` / `useLayoutEffect` 의 setup + cleanup (`setup → cleanup → setup`)
  * `ref` 콜백의 연결/해제
  * `useState`의 초기화 함수와 `setState(prev => ...)` 의 업데이트 함수

> Vite로 만든 프로젝트의 `main.jsx` 를 보면 `import { StrictMode } from 'react'` 로 앱을 감싸고 있습니다.
> 즉 **기본값이 StrictMode ON** 이라서, “왜 마운트 시 fetch가 두 번 나가지?” 는 버그가 아니라 이 검사 때문입니다.
...
## 10. 부록: 클래스 라이프사이클 ↔ Hook 대응표

옛 자료나 레거시 코드를 읽을 때만 필요한 표입니다. **신규 코드는 함수 컴포넌트 + Hook 기준으로 작성합니다.**

| 클래스 메서드 | 함수 컴포넌트에서의 대응 |
| --- | --- |
| `constructor` | `useState(초기값)` / `useState(() => 초기값)` |
| `componentDidMount` | `useEffect(() => {...}, [])` |
| `componentDidUpdate` | `useEffect(() => {...}, [deps])` |
| `componentWillUnmount` | `useEffect(() => () => {...}, [])` 의 cleanup |
| `shouldComponentUpdate` | `React.memo` (+ `useMemo` / `useCallback`) |
| `getDerivedStateFromProps` | 렌더 중에 계산하거나, `key` 로 컴포넌트를 리셋 |
| `getSnapshotBeforeUpdate` | `useLayoutEffect` 에서 DOM 측정 |
| `componentDidCatch` / `getDerivedStateFromError` | **대응 Hook 없음** — 에러 바운더리는 여전히 클래스가 필요 |

### deprecated 된 메서드

`componentWillMount`, `componentWillReceiveProps`, `componentWillUpdate` 는
Concurrent 렌더링에서 **Render Phase가 중단·재시작될 수 있어** 안전하지 않습니다.
현재는 `UNSAFE_componentWillMount` 처럼 **`UNSAFE_` 접두사가 붙은 이름만** 남아 있고, 신규 코드에서는 사용하지 않습니다.

### 1:1 번역이 아니라는 점

`componentDidUpdate` 를 `useEffect(..., [deps])` 로 “번역”하려 들면 대부분 코드가 꼬입니다.
Effect는 **생명주기의 한 시점**이 아니라 **“이 값과 외부 시스템을 동기화한다”** 는 선언으로 이해하는 편이 정확합니다.
그래서 “업데이트일 때만 실행하고 싶다”는 요구가 나오면, 그 로직은 대개 effect가 아니라 **이벤트 핸들러**에 있어야 합니다.
