...
React 17+ 부터 **기본값**이 된 **새 JSX 트랜스폼(Automatic 런타임)** 에서는 실제로 이렇게 변환됩니다:

```js
// 변환 후 (React 17+ ~ React 19의 기본 방식)
import { jsx as _jsx } from "react/jsx-runtime";

const element = _jsx('h1', { className: 'title', children: 'Hello, JSX' });
```

두 방식의 차이:

| 항목 | Classic (`React.createElement`) | Automatic (`react/jsx-runtime`) |
| --- | --- | --- |
| 호출 함수 | `React.createElement` | children이 1개 이하면 `jsx`, 여러 개면 `jsxs` |
| children 위치 | 세 번째 이후 아규먼트 | `props.children` 안 |
| `key` | props 객체 안에 섞임 | **별도 아규먼트**로 분리 전달 |
| `import React` | **필요** | **불필요** (컴파일러가 자동 import) |

> 그래서 **“JSX를 쓰려면 파일 맨 위에 `import React from 'react'` 를 써야 한다”는 말은 React 16 이하 기준의 옛 설명**입니다.
> Vite + React 19 환경에서는 필요 없고, `useState` 처럼 실제로 사용하는 API만 import 하면 됩니다.

어느 방식이든 개념은 같습니다. JSX는 결국

* `type` (DOM 태그 문자열 또는 컴포넌트 함수/클래스)
* `props` 객체 (children 포함)

를 넘겨 **React Element 객체를 만드는 함수 호출**입니다.
아래 설명에서는 구조가 눈에 잘 보이는 `React.createElement` 형태를 계속 쓰지만, **실제 빌드 결과는 `jsx()` / `jsxs()` 호출**이라는 점을 기억해 두세요.
...
* 이벤트 이름은 **camelCase** (`onclick` X, `onClick` O)
* 값으로 **함수(참조)** 를 넘겨야 합니다.

  * `onClick={handleClick}` O
  * `onClick={handleClick()}` X (렌더링 시점에 바로 실행됨)

### 6.6 React 19에서 달라진 attribute 규칙

* **`ref` 가 일반 prop이 되었습니다.** 함수 컴포넌트에 `ref` 를 그대로 넘길 수 있고, `forwardRef` 로 감쌀 필요가 없습니다.
...
<Input ref={inputRef} />   // ✅ React 19
```

* **Context는 `<Context>` 자체를 Provider로 사용**할 수 있습니다.

```jsx
const ThemeContext = createContext('light');

<ThemeContext value="dark">...</ThemeContext>                     // ✅ React 19
<ThemeContext.Provider value="dark">...</ThemeContext.Provider>   // 구방식(아직 동작)
```

* **`propTypes` 와 함수 컴포넌트의 `defaultProps` 는 제거**되었습니다. 기본값은 매개변수 기본값으로 표현합니다.

```jsx
function Card({ theme = 'light', title }) { ... }
```
...
* **형제 사이에서만** 유일(unique)하면 됩니다. 앱 전체에서 유일할 필요는 없습니다.
* `key` 는 **props가 아닙니다.** React가 가져가 버리므로 자식 컴포넌트 안에서 `props.key` 로 읽을 수 없습니다. 값이 필요하면 `<Item key={item.id} id={item.id} />` 처럼 따로 넘기세요.
* 렌더할 때마다 값이 바뀌는 것(`Math.random()`, `Date.now()`)을 key로 쓰면 매번 **DOM을 파괴하고 새로 만들어** 상태가 초기화됩니다.
* **index를 key로 쓰는 것은 가급적 피합니다.**

  * 목록의 **순서가 바뀌거나 중간에 추가/삭제**되면 같은 index에 다른 항목이 오게 되어, React가 “같은 요소”로 착각하고 이전 DOM과 state를 그대로 재사용합니다.
  * 대표적인 증상: 리스트 맨 앞에 항목을 추가했더니 **체크박스 체크 상태나 input에 입력한 값이 엉뚱한 줄에 남아 있는** 현상.
  * 반대로 **정렬·추가·삭제가 전혀 없는 정적인 목록**이라면 index key도 괜찮습니다.
* Fragment에 key가 필요하면 축약형(`<>`)은 쓸 수 없고 `<React.Fragment key={id}>` 형태로 써야 합니다.
...
* `false` → `false` (React는 `false`, `null`, `undefined` 를 렌더링하지 않음)

#### `&&` 의 대표적인 함정: 숫자 `0`

`&&` 는 “왼쪽이 falsy면 **왼쪽 값 자체**를 반환”합니다. 그런데 React는 `false`/`null`/`undefined` 는 무시하지만 **숫자 `0` 은 그대로 화면에 그립니다.**

```jsx
const items = [];

// ❌ items.length가 0이면 화면에 "0"이 찍힌다
...
빈 문자열 `''` 도 같은 이유로 주의가 필요합니다. **`&&` 왼쪽에는 항상 boolean을 두는 습관**을 들이세요.
...
* 빌드 시에는 모두 Element 생성 함수 호출로 변환됩니다. React 17+ 의 기본값은 `react/jsx-runtime` 의 `jsx(type, props)` / `jsxs(type, props)` 이고, `React.createElement(type, props, ...children)` 는 그 이전의 방식입니다. (따라서 JSX만 쓰려고 `React` 를 import 할 필요는 없습니다.)
