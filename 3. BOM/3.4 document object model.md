...
<img src="./images/dom_rendering.png" style="width:80%; height:auto;" />

## 1 DOM이란?

> **DOM(Document Object Model)은 HTML/XML** 과 같은 문서를 프로그램에서 다룰 수 있도록 Document, Element, Text 등의 객체와 이들의 트리 관계를 정의한 표준 객체 모델 및 API입니다.

브라우저는 HTML을 파싱하면서 이 모델에 따라 실제 DOM 객체들을 생성하고 문서 트리를 구성합니다.
...
> <br>
> 헷갈리기 쉬운 경계: 
> "DOM 표준과 HTML 표준은 서로 다른 명세" 입니다.
>
> | 무엇                                                          | 정의하는 표준                    |
> | ----------------------------------------------------------- | -------------------------- |
> | `Node`, `Element`, `Document`, `Attr`, `Text`, 트리 구조, 이벤트 전파 | **DOM Standard**  |
> | `HTMLElement`, `HTMLInputElement.value`, `<img>` 같은 태그의 의미   | **HTML Standard** |
> | 태그를 실제 트리로 바꾸는 **HTML 파싱 알고리즘**                             | **HTML Standard** |
> | `getComputedStyle`, `CSSStyleDeclaration`(=CSSOM)            | **CSS 관련 명세**  |
>
> 즉 "DOM은 HTML 명세다"라는 말은 정확하지 않습니다. DOM은 **문서 종류와 무관한 범용 노드 트리 모델**이고,
> 브라우저는 HTML 소스를 파싱하고 HTML 파싱 규칙에 따라 DOM Tree를 구성합니다
<br>
...
## 3 DOM 구성 요소 (Node Interface 계층 구조)
...
> DOM 변경은 동기적으로 반영되지만 화면 픽셀은 반드시 그 순간 다시 그려지는 것이 아닙니다. 브라우저는 변경 사항을 모아 다음 적절한 rendering opportunity에서 Style/Layout/Paint/Composite 작업을 수행할 수 있습니다.
...
> WebKit 시절 명칭이 널리 퍼져 지금도 자료마다 Render Tree / Layout Tree / Frame Tree(Gecko)로 제각각 불리지만, 오늘날 Chromium(Blink)의 공식 단계 이름은 
> **Style → Layout → Paint → Composite** 이고 그 산출물이 **Layout Tree**입니다.
...
## 8 Shadow DOM, Virtual DOM, Custom Elements (고급 확장)
...
Virtual DOM은 브라우저 표준이 아니라 React 같은 라이브러리가 사용하는 애플리케이션 레벨의 추상화입니다. UI 상태를 JavaScript 객체 구조로 표현하고, 이전 렌더 결과와 새로운 결과를 비교하여 실제 DOM에 필요한 변경을 계산합니다.
