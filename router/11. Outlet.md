...
React Router 없이 단순히 컴포넌트만 쓴다면:

* `Layout` 컴포넌트 안에 헤더/사이드바를 두고
* `Layout`의 children으로 다른 페이지 컴포넌트를 직접 넣으면 됩니다.
...
그런데 **라우팅 시스템**이 들어오면 상황이 달라집니다.

* URL에 따라 어떤 “페이지 컴포넌트”가 children 자리에 들어갈지
* 그 결정은 **React Router가 하길** 원하고,
* 우리는 “여기다가 그려줘”라는 자리를 **Declarative하게** 표시하고 싶습니다.

그래서 나온 것이 바로 `<Outlet />`입니다.

* **컴포넌트는 레이아웃을 만들고**
* **React Router는 `<Outlet />` 위치에 “현재 매칭된 자식 라우트”를 꽂아 넣습니다.**

즉, `<Outlet />`은:

> “URL과 라우트 설정에 의해 선택된 자식 라우트 컴포넌트가 실제로 렌더링되는 자리” 🔌
