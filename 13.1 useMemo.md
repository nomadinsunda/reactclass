...
* `create`
  ...
  * 규칙: **부수 효과(사이드 이펙트)를 넣으면 안 됨**
    ...
    * `fetch`, `setTimeout`, DOM 조작, `setState` 등은 X
...
useEffect(() => {
  onSearch(keyword, options);
}, [keyword, options, onSearch]); // ✅ options는 항상 같은 참조 (onSearch도 deps에!)
```

* `options`는 **마운트 이후 항상 동일한 참조** 유지
* 이제 `keyword`가 바뀔 때만 `useEffect` 실행

> 원래 코드의 deps에는 **`onSearch`가 빠져 있었습니다.**
> 콜백 안에서 쓰는 **props/state는 전부** deps에 들어가야 합니다(`exhaustive-deps`).
> 그리고 `onSearch`가 부모에서 매 렌더 새로 만들어진다면, 이번엔 **그쪽이** effect를 다시 돌립니다.
> → 부모에서 `useCallback`으로 고정하거나, 함수를 effect 안에서 정의해야 합니다.

**그런데 이 예제, 사실 `useMemo`가 정답이 아닙니다**

`{ debounce: 300, minLength: 2 }`는 **아무것에도 의존하지 않는 상수**입니다.
상수를 `useMemo`로 감싸는 것은, **컴포넌트 밖으로 한 줄 옮기면 공짜로 얻을 수 있는 것**을
“훅 슬롯 + deps 비교”라는 비용을 내고 사는 셈입니다.

```tsx
// ✅ 가장 싼 해법: 모듈 스코프 상수 (참조가 영원히 고정, 훅 비용 0)
const SEARCH_OPTIONS = { debounce: 300, minLength: 2 };

function SearchBox({ onSearch }) {
  const [keyword, setKeyword] = useState("");

  useEffect(() => {
    onSearch(keyword, SEARCH_OPTIONS);
  }, [keyword, onSearch]);
}
```
...
1. 값이 **상수**인가? → **모듈 스코프로 이동** (훅 비용 0)
2. 객체를 통째로 넘길 필요가 있나? → **원시값으로 쪼개서 전달** (`debounce={300}`)

   * deps에도 객체 대신 `user.id`, `list.length` 같은 **원시값**을 넣는 습관이 같은 맥락입니다.
   * 객체를 deps에 넣는 순간, 그 객체를 만든 쪽까지 전부 메모이제이션해야 하는 **연쇄**가 시작됩니다.
3. 위 둘이 안 될 때만 → **`useMemo`**
...
* `deps`를 제공하지 않으면?
  ...
  * `useMemo(fn)` → 매 렌더 `create`가 다시 실행되고 **매번 새 값**이 나옵니다.
    캐시 효과는 0인데 훅 비용만 남으므로 **사실상 쓸 이유가 없습니다.**
  * 공식 문서에서도 보통 **항상 deps를 제대로 넣는 것**을 권장

**그리고 가장 중요한 포인트 – 이 캐시는 “보장”이 아닙니다**

위 의사코드만 보면 “deps가 같으면 무조건 이전 값이 나온다”고 오해하기 쉽습니다.
하지만 리액트 공식 문서는 이렇게 못 박습니다.

> `useMemo`는 성능 최적화 수단이며, **리액트는 특별한 이유가 있으면 캐시를 버릴 수 있습니다.**
> (예: 화면 밖으로 밀려난 서브트리의 메모리 회수)
> 따라서 코드는 **`useMemo` 없이도 똑같이 동작해야** 합니다.

실전에서 이게 의미하는 바:

```tsx
// ❌ "한 번만 만들어진다"에 의존하는 코드 – useMemo는 이 용도가 아닙니다
const socket = useMemo(() => new WebSocket(url), [url]);
const id = useMemo(() => crypto.randomUUID(), []);
```

* 캐시가 버려지면 **소켓이 다시 열리고, id가 바뀝니다.**
* **“반드시 한 번”이 보장되어야 하는 값**은 `useMemo`가 아니라

  * `useState(() => 초기값)`의 **지연 초기화**
  * `useRef` + 최초 1회 초기화
  * 고유 id라면 `useId`

  로 풀어야 합니다.

 정리: **`useMemo`는 “속도 힌트”이지 “실행 횟수 계약”이 아닙니다.**
`useMemo`를 전부 지웠을 때 화면이 느려지기만 한다면 정상,
**동작이 달라진다면 그건 최적화가 아니라 버그**입니다.
...
* `useMemo`는 콜백을 **실행해서 나온 결과**를 저장하므로, `() => fn`을 실행하면 결과가 `fn` 자신입니다.
* 그래서 두 훅은 **같은 메커니즘의 다른 포장**입니다. `useCallback`은 이 패턴을 위한 문법 설탕에 가깝습니다.

**여기서 꼭 짚고 갈 오해**

> “`useMemo`/`useCallback`을 쓰면 그 안의 값/함수가 매 렌더 만들어지지 않는다” → **틀렸습니다.**

```jsx
const handle = useCallback(() => doSomething(a), [a]);
// 👆 () => doSomething(a) 라는 함수 리터럴은 렌더마다 새로 생성되어 인자로 전달됩니다.
//    useCallback은 그중 하나를 골라 "반환값의 참조"만 고정할 뿐이고, 나머지는 버려집니다.
```

* `useMemo(() => ({ a }), [a])`도 마찬가지로 **화살표 함수 자체는 매 렌더 생성**됩니다.
  (다만 `deps`가 같으면 안쪽 객체 생성 `{ a }`는 실행되지 않습니다.)
* 즉 두 훅이 절약하는 것은 **생성 비용(대개 무시할 수준)** 이 아니라,
  **참조가 바뀌었을 때 뒤따라오는 작업**(memo 자식 리렌더, effect 재실행, 리스너 재등록, 비싼 계산)입니다.

 그러므로 **뒤따라오는 작업이 없다면 메모이제이션의 이득은 정확히 0**이고,
남는 것은 비교 비용과 메모리뿐입니다.
...
* 복잡도 표기(O(n) 등)가 아니라 **실제로 걸린 시간(ms)** 으로 판단하세요.
  원소가 10개인 O(n²)보다 원소가 10만 개인 O(n)이 훨씬 느립니다.
* 판단 방법은 아주 단순합니다. **먼저 `useMemo` 없이 재 보세요.**
  ...
  * **1ms 미만** → 감싸지 마세요. `useMemo` 자체 비용이 더 큽니다.
  * **수 ms 이상 + 렌더가 잦음** → `useMemo` 후보
* 성능 이슈가 **실제로** 발생했을 때,

  * React DevTools **Profiler**로 병목 컴포넌트를 특정하고
    (설정에서 “Record why each component rendered”를 켜면 렌더 원인까지 보입니다)
  * **핫스팟 지점에만** 신중하게 적용하고
  * 적용 후 **다시 측정**해서 실제로 좋아졌는지 확인합니다. 아니면 되돌립니다.

**순서를 지키세요**

> **① 측정 → ② 렌더 원인 파악 → ③ 구조 개선 → ④ 그래도 남으면 메모이제이션**

③의 구조 개선이란:

* 자주 바뀌는 **state를 실제로 쓰는 하위 컴포넌트로 내리기**
* 무거운 서브트리를 **`children`으로 넘겨** 부모 렌더와 분리하기
* 자주 바뀌는 값과 안 바뀌는 값의 **컨텍스트 분리**, 리스트 **가상화**

구조로 풀 수 있는 문제를 `useMemo`로 덮으면,
비용은 비용대로 내면서 원인은 그대로 남습니다.
...
* `useMemo`는 **캐시 힌트**다 → 전부 지워도 **동작은 같아야** 한다
* `useMemo`는 **리렌더를 막지 못한다** → 그건 `React.memo`(그리고 구조 개선)의 몫

**4⃣ 한 걸음 더 – React Compiler**

* `React Compiler`는 이런 수동 `useMemo`/`useCallback`을 **컴파일 시점에 자동으로** 넣어 줍니다.
* 다만 **React 19에 자동 내장된 기능이 아니라, 빌드 설정에 직접 추가하는 별도 opt-in 도구**입니다.
  (설치하지 않으면 이 장의 수동 최적화가 그대로 필요합니다.)
* 컴파일러는 **“Rules of React”를 지키는 코드**에만 안전하게 동작하고,
  규칙 위반이 의심되면 해당 컴포넌트의 최적화를 조용히 건너뜁니다.
* 자세한 내용은 **「13. Memoization」의 React Compiler 절**을 참고하세요.
