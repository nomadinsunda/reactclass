...
* 첫 번째 인수: `type` (DOM 태그 문자열 또는 컴포넌트 함수/클래스)
* 두 번째 인수: `props` 객체
* 세 번째 이후 인수: `children`들

을 인수로 가지는 함수 호출 문법입니다.
...
이 한 줄은 **“Card라는 컴포넌트(함수)를 어떤 인수로 호출하느냐”** 를 정의하는 문법입니다.
...
그리고 React 내부에서는 이 세 번째 이후 인수들을 모아서
...
> `<Card ...>...</Card>`
> == `Card` 함수(컴포넌트)를 **특정 인수(props, children)** 로 호출하는 **문법적 설탕(syntax sugar)**
