...
**“훅(useEffect, useLayoutEffect 등)이 언제, 어떤 순서로 실행되는가”**를 이해하는 것과 같습니다. 🔄

즉, 이제는 **메서드 이름(componentDidMount 같은 것)** 이 아니라
➡️ **렌더링 단계(Render/Commit) + 훅 실행 타이밍 + cleanup**
...
### 4-3. 자식 자신의 state 변경 (Update – 자기 자신으로 인한 재렌더) 🔁
...
이렇게 커스텀 훅을 만들어두면,
학생들에게 “라이프사이클 관점”을 더 직관적으로 설명하기 좋습니다. 👍
