...
3. **네이티브 이벤트로의 통로: `event.nativeEvent`**

   * SyntheticEvent는 브라우저가 만든 원본 이벤트(`MouseEvent`, `KeyboardEvent` …)를 **감싼 크로스브라우저 래퍼**입니다.
   * 원본이 필요하면 언제든 `event.nativeEvent`로 꺼낼 수 있습니다.

   ```jsx
   function handleClick(e) {
     console.log(e.nativeEvent instanceof MouseEvent); // true
     e.nativeEvent.stopImmediatePropagation(); // SyntheticEvent에는 없는 메서드
   }
   ```

4. **이벤트 풀링(Event Pooling)은 이제 없습니다 (React 17에서 제거)**

   * React 16까지는 성능 최적화를 위해 SyntheticEvent 객체를 **재사용(pooling)** 했고,
     핸들러가 끝나면 프로퍼티가 `null`로 초기화되어 비동기 코드에서 쓰려면 `event.persist()`가 필요했습니다.
   * **React 17에서 이벤트 풀링이 완전히 제거**되었고, React 18/19도 마찬가지입니다.
     이벤트 객체는 더 이상 재활용되지 않으므로 `setTimeout`이나 `await` 이후에도 그대로 읽을 수 있습니다.
   * `event.persist()`는 하위 호환을 위해 남아 있을 뿐 **아무 동작도 하지 않는 no-op**입니다. 새 코드에서는 쓰지 마세요.

   ```jsx
   function handleClick(e) {
     // React 17+ : persist() 없이도 안전
     setTimeout(() => console.log(e.type, e.target), 1000);
   }
   ```

   * 단, **`e.currentTarget`은 예외**입니다. 전파가 끝나면 `null`로 되돌아가므로
     비동기에서 쓰려면 핸들러 안에서 미리 변수에 담아두세요.

---

## 4. React의 이벤트 위임(Event Delegation) 구조
...
### 4-1. 위임 대상은 `document`가 아니라 **root container** (React 17에서 변경)

* **React 16까지**: 거의 모든 이벤트를 **`document`** 에 위임했습니다.
* **React 17부터(18·19 동일)**: `createRoot(container)`의 **그 `container`(예: `#root`)** 에 위임합니다.

```js
// React 17+ 가 실제로 하는 일 (개념적으로)
rootContainer.addEventListener('click', dispatchDiscreteEvent, true);  // 캡처용
rootContainer.addEventListener('click', dispatchDiscreteEvent, false); // 버블용
```

왜 바꿨을까요?

* 한 페이지에 **React 버전이 다른 여러 앱을 점진적으로 이식(gradual upgrade)** 할 수 있게 하려고
* jQuery 등 다른 라이브러리, 혹은 바깥에서 `document`에 건 리스너와의 **`stopPropagation` 충돌**을 줄이려고

> 아직도 “React는 모든 이벤트를 `document`에 붙인다”라고 쓰인 자료가 많은데, **React 17 이후로는 틀린 설명**입니다.

### 4-2. 네이티브 리스너와 섞어 쓸 때의 실행 순서 함정

React 핸들러는 **루트 컨테이너**에서 실행되므로, 루트 *안쪽* 요소에 `addEventListener`로 직접 건 리스너와 순서가 이렇게 됩니다.

```
1) root 바깥(window/document)의 캡처 리스너
2) root의 캡처 리스너      → React의 onClickCapture 들이 여기서 위 → 아래로 실행
3) root 안쪽 요소들의 네이티브 리스너 (캡처 → 타깃 → 버블)
4) root의 버블 리스너      → React의 onClick 들이 여기서 아래 → 위로 실행
5) document / window의 버블 리스너
```

결과적으로:

* **React의 `onClickCapture`는 안쪽 네이티브 리스너보다 먼저**,
  **React의 `onClick`은 안쪽 네이티브 리스너보다 나중에** 실행됩니다.
* 자식 DOM 노드에 직접 건 네이티브 리스너에서 `e.stopPropagation()`을 호출하면
  이벤트가 루트까지 올라가지 못해 **부모 컴포넌트의 `onClick`이 통째로 죽습니다.**
* 반대로 React 핸들러에서 `e.stopPropagation()`을 호출해도, 그건 **React 내부의 합성 전파만** 멈춥니다.
  `document`/`window`에 직접 건 네이티브 리스너는 **그대로 실행**됩니다. (막으려면 `e.nativeEvent.stopImmediatePropagation()`)
...
React는 루트에서 이벤트를 한 번 받은 뒤, 타깃 DOM 노드에 대응하는 **Fiber 노드에서 위로 거슬러 올라가며**
`onClick`/`onClickCapture` 핸들러 목록을 모으고, 그 목록을 순서대로 실행합니다.
즉 전파를 **DOM 트리가 아니라 React 트리 기준으로 재현**하는 것입니다.

> 그래서 `createPortal`로 DOM상 완전히 다른 위치(예: `document.body` 바로 아래)에 렌더링해도,
> 이벤트는 **JSX상의 부모 컴포넌트로 버블링**됩니다. 모달을 포털로 빼도 상위 `onClick`이 잡히는 이유가 이것입니다.
...
## 6. `preventDefault()`와 `return false`의 차이
...
* DOM: `return false`는 **인라인 `onclick=""` / `elem.onclick = fn` 방식에서만** `preventDefault()` 효과가 있습니다.
  (전파까지 함께 막아주는 건 **jQuery의 `return false`**이고, `addEventListener`로 등록한 리스너에서는 **아무 효과도 없습니다.**)
* React: `return false`는 **아무 의미 없음**,
  **항상 `event.preventDefault()`와 `event.stopPropagation()`을 직접 호출**해야 합니다.

> **React 19의 예외**: `<form action={serverAction}>`처럼 **`action` prop에 함수를 넘기면**
> React가 폼 제출을 가로채 **기본 동작을 자동으로 막고**(별도 `preventDefault()` 불필요) 그 함수를 실행합니다.
> 반면 `onSubmit`으로 직접 처리할 때는 예전과 똑같이 `event.preventDefault()`를 호출해야 합니다.
...
* 진짜 DOM `change`(포커스가 빠질 때) 타이밍이 필요하면 `onBlur`를 쓰거나 네이티브 리스너를 직접 붙여야 합니다.

### 7-2. “React에서는 버블링이 다르게 동작하는” 예외 이벤트들

| 이벤트                             | 브라우저 DOM     | React                                                        |
| ------------------------------- | ----------- | ------------------------------------------------------------ |
| `focus` / `blur`                | **버블링 X**   | `onFocus`/`onBlur`는 내부적으로 `focusin`/`focusout` 기반이라 **버블링 O** |
| `scroll`                        | 요소에서는 버블링 X | `onScroll`도 **버블링하지 않음** (React 17에서 위임에서 제외)                 |
| `mouseenter` / `mouseleave`     | 버블링 X       | `onMouseEnter`/`onMouseLeave`도 **버블링하지 않음** (React가 별도로 시뮬레이션) |
| 미디어 이벤트(`onPlay`, `onEnded` … ) | 버블링 X       | 위임하지 않고 **해당 DOM 노드에 직접** 리스너를 붙임                             |

* 그래서 “상위에서 한 번에 스크롤을 감지”하는 위임 패턴은 React에서 동작하지 않습니다.
  스크롤되는 요소 자신에게 `onScroll`을 달아야 합니다.
